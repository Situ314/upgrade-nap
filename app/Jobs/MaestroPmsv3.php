...
class MaestroPmsv3 implements ShouldQueue
{
                ...
                if ($guest_checkin_details) {
                    ...
                    if ($guest) {
                        ...
                        $guest_checkin_details = GuestCheckinDetails::where('status', 1)
                            ...
                            ->where(function ($q) use ($guest_id, $check_in, $check_out) {
                                $q
    ...
    private function __offmarket(
        ...
        if ($HotelRoomsOut) {
            ...
            } else if ($OffmarketFlag == "true" && $OutOfInventoryFlag == "false") {
                $data_to_update = [
    ...
    {
        if ($replace_data) return (isset($string) && is_string($string)) ? str_replace($replace_data["replace"], $replace_data["by"], addslashes($string)) : '';
     ...
     *
     * @param  integer  $hotel_id           // Hotel id
     * @param  integer  $guest_id           // guest id
     * @param  integer  $staff_id           // Staff id
     * @param  string   $email              // Email del huesped
     * @param  string   $phone              // Phone del huesped
     * @param  boolean  $back               // Si el huesped ya ha tenido estadias se le envia un Welcome Back
     * @param  boolean  $welcome            // Enviar Welcome Message de sms chat
     * @param  boolean  $angel              // Enviar invitacion de Angel
     */
