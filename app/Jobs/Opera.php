...
class Opera implements ShouldQueue
{
    ...
    public function getReservationRoom($room_id, $config, $pms_hotel_id)
    {
        ...
        ';
        $curl = curl_init();
        $actions = ($this->hotel_id != 281) && ($this->hotel_id != 266) && ($this->hotel_id != 296) && ($this->hotel_id != 238) && ($this->hotel_id != 314) && ($this->hotel_id != 289) && ($this->hotel_id != 303) && ($this->hotel_id != 443) && ($this->hotel_id != 207) ? [
            'Content-Type: text/xml;charset=UTF-8',
            ...
            if ($data) {
                ...
                foreach ($fetch_room as $room) {
                    ...
                    if ($add) {
                        ...
                        $this->customWriteLog('sync_opera', $this->hotel_id, Arr::get($room, 'FrontOfficeStatus'));

                        // && $this->hotel_id != 289
                        if (Arr::get($room, 'FrontOfficeStatus') == 'OCC') {
                            // $room_status = 'Clean';
