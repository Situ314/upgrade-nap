            ...
            try 
            {
                ...
                else 
                {
                    ...
                    if( $guest ) 
                    {
                        ...
                        ->where( function( $query ) use ( $guest_id, $check_in, $check_out ) {
                            $query                            
            ...
            foreach ($arr_check_in as $data) {
                ...
                if( $room_no > 0 )
                {
                    ...
                    if ( $find_guest_checkin_details ) 
                    {
                        ...
                        ->where( 'hotel_id' , $hotel_id )
                        ->where( 'guest_id' , $guest_id )                
                        ->where( function( $query ) use ( $reservation_number_key, $check_in, $check_out ) {
                            $query
                            ->whereRaw("'$check_in' BETWEEN check_in and check_out")
                            ->orWhereRaw("'$check_out' BETWEEN check_in and check_out");
                        })
    ...
    public function unique_inArray($hotel_id, $array) 
    {
        try {
            ...
            foreach(array_reverse($array) as $val) {
                ...
                if(isset($val->ReservationNumberKey)        && is_string($val->ReservationNumberKey)        && !empty($val->ReservationNumberKey))       { $reservation_number_key        = $this->proccessString($val->ReservationNumberKey); }
            ...
            try {
                ...
                foreach ( $offMarket as $key => $value ) {
                    if(
                        ...
                        }else if( is_array($value->OffmarketText->Text) && count($value->OffmarketText->Text) > 0 ) {
                            $__data = $value->OffmarketText->Text;
                        }
                        ...
                        if($HotelRoomsOut) {
                            ...
                            } else if($value->OffmarketFlag == "true" && $value->OutOfInventoryFlag == "false") {
                                $HotelRoomsOut->fill([
