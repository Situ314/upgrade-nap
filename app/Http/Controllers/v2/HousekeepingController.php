...
class HousekeepingController extends Controller
{
    ...
    public function show(Request $request, $id)
    {
        ...
        ->where('is_active', 1)
        ->with(['Room' => function( $q ) { $q->select('location','room_id'); }])
        ->first();
    ...
    public function pickup(Request $request)
    {
        ...
        try {
            ...
            $validation = Validator::make($pickup_event, [
                ...
                'dept_id'   => ['numeric', Rule::exists('departments')->where(function($q) use ($hotel_id) { $q->where('hotel_id', $hotel_id); }) ],
                'tag_id'    => ['numeric', Rule::exists('tags')->where(function($q) use ($hotel_id) { $q->where('hotel_id', $hotel_id); }) ],
