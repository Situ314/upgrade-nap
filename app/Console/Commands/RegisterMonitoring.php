...
class RegisterMonitoring extends Command
{
    /**
    ...
    public function SearchLogsMaestro($tableName, $hotel_id, $pms_hotel_id)
    {
        try {
            ...
            $date = date('Y-m-d');
            $hotel_monitoring = Monitoring::where('Hotel_id', $hotel_id)
                ->whereDate('date', $date)
                ->orderby('date', 'desc')
                ->orderby('time', 'desc')
                ->first();
            ...
            if ($count_monitoring > 0) {
                $total = $total->whereDate('created_on_Date', $date);
            }
            // Agregar filtro de rango de fechas
            if ($start_time) {
                $total = $total->whereTime('Created_on_Time', '>=', $start_time)->whereTime('Created_on_Time', '<=', $end_time);
            }
