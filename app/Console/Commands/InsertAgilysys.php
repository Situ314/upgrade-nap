...
class InsertAgilysys extends Command
{
    ...
    {
        //sleep(600);
        $IntegrationsActives = IntegrationsActive::where(function( $query ) { $query->where('int_id', 11)->where('state', 1); })->get();
    ...
    public function ClearArray( $stays, $config, $hotel_id ) {
        try {
            ...
            foreach (array_reverse($stays) as $stayskey => $stay) {
                ...
                } else if( isset($stay["arrivalDate"]) && !empty($stay["arrivalDate"]) ) {
                ...
                if( isset($stay["checkInTime"]) && !empty($stay["checkInTime"]) ) {
                    if( \DateTime::createFromFormat('m-d-y H:i:s', $stay["checkInTime"]) != false ) {
                        ...
                        // g:i:s
                    } else if(\DateTime::createFromFormat('m/d/Y g:i:s A', $stay["checkInTime"]) != false) {
                        $__start_have_time = true;
                ...
                } else if( isset($stay["departureDate"]) && !empty($stay["departureDate"]) ) {
                ...
                if( isset($stay["check_out_time_2"]) && !empty($stay["check_out_time_2"]) ) {
                    ...
                    }else if(\DateTime::createFromFormat('m/d/Y g:i:s A', $stay["check_out_time_2"]) != false) {
                ...
                if(!empty(trim($__start)) && !empty(trim($__end))) {
                    if(strtotime($__start) < strtotime($__end)) {
                        if(!$__start_have_time) { $__start .= " $config[check_in_time]"; }
                        if(!$__end_have_time) { $__end .= " $config[check_out_time]"; }                    
    
                    } else if(strtotime($__start) == strtotime($__end)) {
                        if(!$__start_have_time) { $__start .= " $config[check_in_time]"; }
                        if(!$__end_have_time) { $__end .= " $config[check_out_time_2]"; }                     
        ...
        foreach ($stays as $stayKey => $stay) {
            ...
            try {
                ...
                if( $GuestCheckinDetails ) {
                    ...
                    $room_id = 0;
                    if(!empty($stay["roomNumber"])) { $room_id = (int)$this->findRoomId($hotel_id, $staff_id, trim($stay["roomNumber"])); }
                    ...
                    //if( $email_phone_valid ) {
                        ...
                        $room_id = 0;
                        if(!empty($stay["roomNumber"])) { $room_id = (int)$this->findRoomId($hotel_id, $staff_id, $stay["roomNumber"]); }
