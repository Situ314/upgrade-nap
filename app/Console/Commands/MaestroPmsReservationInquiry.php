...
class MaestroPmsReservationInquiry extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'maestroPms:requestInquiry';
    ...
    {
        $hotels = [ 198/*, 208, 216, 217, 230, 231, 232*/ ];
        $__response = [];
                            ...
                            if(isset($Reservations) && isset($Reservations->ReservationData)) {
                                ...
                                if(isset($ReservationData->ReservationStatus) && !empty($ReservationData->ReservationStatus)) {
                                    ...
                                    if (isset($ReservationData->Room) && isset($ReservationData->Room->RoomCode) && is_string($ReservationData->Room->RoomCode) && !empty($ReservationData->Room->RoomCode)) {
                                        ...
                                        $room       = $this->findRoomId($this->hotel_id, 1, $room_code);
                                    ...
                                    Mail::raw("Prueba", function(Message $message) {
                                        $message->to(['jsanchez@mynuvola.co'])
                                        ->from('admin@mynuvola.net')
        ...
        Mail::send('emails.maestro',[], function($m) use ($emails){
    ...
    private function findRoomId($hotel_id, $staff_id, $room_code)
    {
        ...
        if(is_numeric($room_code)) {
            ...
            $HotelRoom = \App\Models\HotelRoom::where('hotel_id', $hotel_id)
            ->where('active', 1)
        ...
        $HotelRoom = \App\Models\HotelRoom::create([
            'hotel_id'      => $hotel_id,
            'location'      => $room_code,
            'created_by'    => $staff_id,
